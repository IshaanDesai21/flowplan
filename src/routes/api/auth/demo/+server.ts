...
export const POST: RequestHandler = async (event) => {
	try {
		const demoId = uuid().slice(0, 8);
		const user = await db.user.create({
			data: {
				email: `demo-${demoId}@flowplan.app`,
				name: 'Demo User',
				isDemo: true,
				provider: 'demo'
			}
		...
		await db.meeting.create({
			data: {
				...
				participants: ['alice@acme.com', 'bob@acme.com', 'you'],
		...
		return json({ error: 'Could not create demo account' }, { status: 500 });
	}
};
