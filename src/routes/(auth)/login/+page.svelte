...
<div class="min-h-screen bg-page flex items-center justify-center p-4">
	<div class="w-full max-w-md animate-scale-in">
		<div class="text-center mb-8">
			...
			<p class="text-secondary mt-2">Enter your details to access your account</p>
		</div>
		...
		<Card padding="lg">
				...
				use:enhance={() => {
					...
					return async ({ result, update }) => {
						...
						if (result.type === 'failure') {
							toasts.error(result.data?.message || 'Login failed');
			...
			>
				...
				<div class="flex items-center justify-between pb-2">
					<label class="flex items-center gap-2 text-sm text-secondary cursor-pointer">
						<input type="checkbox" class="rounded border-default text-[var(--color-accent)] focus:ring-[var(--color-accent)] bg-transparent" />
						Remember me
					</label>
					<a href="#" class="text-sm font-medium text-[var(--color-accent)] hover:underline">Forgot password?</a>
				</div>
			...
			<div class="mt-6 flex items-center justify-between gap-4">
				...
				<span class="text-xs text-tertiary uppercase font-medium">Or continue with</span>
				<div class="h-px flex-1 bg-default"></div>
			</div>
		...
		<p class="mt-8 text-center text-sm text-secondary">
			...
			<a href="/signup" class="font-medium text-[var(--color-accent)] hover:underline">Sign up</a>
		</p>
	</div>
</div>
